...
namespace Joe{
	...
	bool SdlInput::IsKeyPressedImpl(Sint32 keycode){
		SDL_Event event;
		//FIXME: gets in way of closeing window
	...
	void SdlInput::InitGamepad(){
		if(SDL_InitSubSystem(SDL_INIT_GAMEPAD)){
        	//FIXME: SDL3::fix connecting and opening gamepad
			if(SDL_HasGamepad()){
				if(SDL_GamepadConnected(m_GamePad)){
					std::string ControllerString = SDL_GetGamepadName(m_GamePad);
					JOE_ENGINE_INFO("SDL::INPUT::GAMEPAD::FOUND");
					JOE_ENGINE_INFO("SDL::INPUT::GAMEPAD::{0}", ControllerString);
				}
			}
		}
	}
